        ...
        if (findUser != null) {
            ...
            if (remember != null && remember.equals(Boolean.TRUE)) {
                String cookieValue = Base64.encode(findUser.getUsername())
                        .concat(SampleSystemConstant.Session.COOKIE_SPLIT).concat(findUser.getPassword());
                CookieUtils.setCookie(data.getResponse(), SampleSystemConstant.Session.REMEMBER_COOKIE_NAME,
                        cookieValue, 15 * 24 * 60 * 60);
            }
            blogContext.setSessionUser(data.getRequest(), data.getResponse(), findUser);
            String redirectUrl = SampleSystemConstant.Session.MANAGEMENT_INDEX;
